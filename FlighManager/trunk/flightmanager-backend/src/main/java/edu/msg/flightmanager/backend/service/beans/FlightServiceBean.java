...
public class FlightServiceBean implements FlightService {
	...
	public FlightDto insert(FlightDto flightDto) throws ServiceException {
		...
		try {
			...
			if (isAvailableForInsert(flight)) {
				flight = flightRepository.insert(flight);
				FlightDto insertedflight = FlightAssembler.modelToDto(flight);
				insertNextFlights(flight.getTemplate(), insertedflight.getDate());
				return insertedflight;
			} else {
				throw new ServiceException("Can't add another flight, beacause exist one.");
			}
		} catch (ValidationException e) {
			throw new ServiceException(e.getMessage());
		} catch (RepositoryException e) {
	...
	private boolean isAvailableForInsert(Flight flight) {
		for (Flight flightDB : flightRepository.getAvailableFlights()) {
			if (flightDB.getTemplate().getCode().equals(flight.getTemplate().getCode()) && flight.getDate().equals(flightDB.getDate())){
				return false;
			}
		}
		return true;
	...
	public void insertNextFlights(FlightTemplate template, Date date) {
		Plane plane = planeRepository.getAvailablePlanes(date).get(0);
	...
	private void generateNextFlights(Flight flight) {
		...
		if (type.equals(PeriodicityType.DAILY)) {
			flightNext.add(Calendar.DAY_OF_YEAR, NUMBER_OF_DAYS_FOR_NEXT_FLIGHT_DAILY+1);
		}

		if (type.equals(PeriodicityType.WEEKLY)) {

			flightNext.add(Calendar.DAY_OF_YEAR, 7 * (NUMBER_OF_WEEKS_FOR_NEXT_FLIGHT_WEEKLY+1));

		}
		if (type.equals(PeriodicityType.MONTHLY)) {
			if (isNextMonth(flight.getDate(), (NUMBER_OF_MONTH_FOR_NEXT_FLIGHT_MONTHLY+1))) {
				flightNext.add(Calendar.MONTH, NUMBER_OF_MONTH_FOR_NEXT_FLIGHT_MONTHLY+1);
			} else {
				return;
			}
		}
	...
	@Override
	public Map<UserDto, Long> reportWithAllFlightHoursOfAllCrewMembers(Date startDate, Date endDate, String userType, String companyName) throws ServiceException {
		...
		try{
			if(userType.equals(UserType.ADMINISTRATOR.toString())) {
				crewMembers = userRepository.getAllCrewMembers();
			} else if(userType.equals(UserType.COMPANY_MANAGER.toString())) {
				List<User> pilots = userRepository.getByTypeAndCompany(UserType.PILOT, companyName);
				List<User> stewards = userRepository.getByTypeAndCompany(UserType.STEWARD, companyName);
				crewMembers.addAll(pilots);
				crewMembers.addAll(stewards);
			}
		} catch (RepositoryException e) {
			throw new ServiceException(e.getMessage());
		}
	...
	@Override
	public void insertNextFlights(FlightTemplateDto templateDto, Date date) throws ServiceException {
		FlightTemplate template = FlightTemplateAssembler.dtoToModel(templateDto);
		insertNextFlights(template, date);
