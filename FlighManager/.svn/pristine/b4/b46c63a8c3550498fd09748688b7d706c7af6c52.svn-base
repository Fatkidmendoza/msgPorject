...
public class PlanesBean implements Serializable {
	...
	private List<PlaneDto> filteredPlanes;
	...
	public List<PlaneDto> getAllPlanes() {
		...
		try {
			if (role.equals(Constants.ADMIN)) {
				...
				return planeService.getByCompany(UsersBean.userInSession().getCompanyName());
			}
	...
	public List<PlaneDto> getFilteredPlanes() {
		return filteredPlanes;
	}

	public void setFilteredPlanes(List<PlaneDto> filteredPlanes) {
	...
	public boolean permissionCompanyAdministrator() {
		return UsersBean.userInSession().getType().equals(Constants.ADMIN);
	}

	public boolean permissionCompanyManager() {
		return UsersBean.userInSession().getType().equals(Constants.COMPANY_MANAGER);
	}

	public String companyNameOfUserInSession() {
		String companyName = UsersBean.userInSession().getCompanyName();
		planeToInsert.setCompanyName(companyName);
		return companyName;
	}
}
