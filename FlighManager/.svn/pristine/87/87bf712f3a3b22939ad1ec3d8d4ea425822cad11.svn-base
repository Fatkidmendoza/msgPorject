...
public class UserServiceBean implements UserService {
	...
	public void changePassword(UserDto userDto) throws ServiceException {
		...
		try {
			...
			user.setPassword(PasswordEncrypter.generateHashedPassword(userDto.getPassword()));
			user = userRepository.update(user);
			tokenRepository.deleteAllHavingUser(user);
		} catch (RepositoryException e) {
	...
	@Override
	public List<UserDto> getByCompany(String companyName) throws ServiceException {
		List<User> users = new ArrayList<>();
		try {
			users = userRepository.getByCompany(companyName);
		} catch (RepositoryException e) {
			throw new ServiceException(e.getMessage());
		}
